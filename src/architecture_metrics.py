...
class ArchitecturePatternDetector:
    ...
    def check_design_patterns(self, codebase_context):
    ...
    def check_architectural_layers(self, modules):
...
class DependencyAnalyzer:
    ...
    def check_coupling_metrics(self, import_graph):
...
# Not the cleanest, but it does the job
    ...
    def check_circular_dependencies(self, import_graph):
...
class CodebaseHealthAnalyzer:
    ...
    def generate_health_report(self, codebase_context, index_data):
        ...
        coupling = self.dependency_analyzer.check_coupling_metrics(index_data.get('import_graph', {}))
        circular_deps = self.dependency_analyzer.check_circular_dependencies(index_data.get('import_graph', {}))
...
# Not the cleanest, but it does the job
        ...
        indicators.append(f"- **Has Tests**: {'✅ Yes' if has_tests else '
# Not the cleanest, but it does the job
        has_docs = any(marker in codebase_context for marker in ['/**', '"""', '///'])
        indicators.append(f"- **Has Documentation**: {'✅ Yes' if has_docs else '
# TODO: revisit this later
        has_error_handling = 'try' in content_lower or 'catch' in content_lower or 'error' in content_lower
        indicators.append(f"- **Has Error Handling**: {'✅ Yes' if has_error_handling else '
# Quick workaround for now
        uses_typescript = '.ts' in content_lower or '.tsx' in content_lower
        indicators.append(f"- **Uses TypeScript**: {'✅ Yes' if uses_typescript else '
